...
var _ planner.CommandsPlanner = &indexplanner.IndexCommandsPlanner{}
...
func (ts *CommandsPlannerTestSuite) newSUT() (planner.CommandsPlanner, *transaction.Transaction, *metadata.Manager, func()) {
	...
	cp := indexplanner.NewIndexCommandsPlanner(mdm)
...
func (ts *CommandsPlannerTestSuite) TestExecuteCreateTable_Fail() {
	...
	require.ErrorIs(t, err, indexplanner.ErrExecuteError)
}
...
func (ts *CommandsPlannerTestSuite) TestExecuteCreateIndex_Fail() {
	...
	assert.ErrorIs(t, err, indexplanner.ErrExecuteError)
...
func (ts *CommandsPlannerTestSuite) TestExecuteDelete_Ok() {
	...
	require.NoError(t, mdm.CreateTable("table1", ts.testLayout().Schema, trx))
	require.NoError(t, mdm.CreateIndex("index1", "table1", indexes.HashIndexType, "id", trx))
...
func (ts *CommandsPlannerTestSuite) TestExecuteUpdate_Ok() {
	...
	require.NoError(t, mdm.CreateTable("table1", ts.testLayout().Schema, trx))
	require.NoError(t, mdm.CreateIndex("index1", "table1", indexes.HashIndexType, "name", trx))
...
func (ts *CommandsPlannerTestSuite) TestExecuteInsert_Ok() {
	...
	assert.Less(t, int64(float64(cnt)*.1), idx.DistinctValues("id"))
	assert.GreaterOrEqual(t, int64(cnt), idx.DistinctValues("id"))
}

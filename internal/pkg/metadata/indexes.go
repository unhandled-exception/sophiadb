...
func (i *Indexes) CreateIndex(idxName string, tableName string, idxType indexes.IndexType, fieldName string, trx records.TSTRXInt) error {
	...
	if err = ts.ForEach(func() (bool, error) {
	...
	err = ts.ForEachField(func(name string, fieldType records.FieldType) (bool, error) {
		var verr error
...
func (i *Indexes) TableIndexes(tableName string, trx records.TSTRXInt) (IndexesMap, error) {
	...
	err = ts.ForEach(func() (bool, error) {
		...
		verr = ts.ForEachField(func(name string, fieldType records.FieldType) (bool, error) {
			var werr error

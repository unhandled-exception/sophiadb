...
.PHONY: test
test: gen
	go test -timeout 30s -race -shuffle on -coverprofile=coverage.out -covermode atomic ./...
	@go tool cover -func coverage.out | grep -E "^total:.+?\d+.\d+%"

.PHONY: test_ci
test_ci:
	go test -timeout 30s -race -shuffle on -coverprofile=coverage.out -covermode atomic -count=1 -v ./...
